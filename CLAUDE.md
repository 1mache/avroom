...
- **Frontend**: `DashboardScreen`'s flask button (Debug Dashboard) and `UploadScreen`'s "Skip validation" switch (styled like `Toolbar.tsx`'s existing `role="switch"` controls) both render only when `user?.is_admin` is true.
