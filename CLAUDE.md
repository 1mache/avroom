...
```
avroom/
├── ai-pipeline/          # The AI pipeline (Python package: avroom_object_removal)
│                         #   Was TestModules/ — renamed; the import namespace still says
│                         #   object_removal, which now understates what it does.
...
│   ├── api/deps.py       # require_object (404) + canvas_writer (409) — see Shared FastAPI Helpers
│   ├── core/             # image_processing.py - bridges API to ObjectRemover
│   │                     #   image_files.py (paths/bytes/decode) + erase_mask.py (lasso mask) sit under it
...
- `api/deps.py` — `require_object(uuid) -> ObjectMetadata` (raises the one canonical 404) and `canvas_writer(uid, action=...)`, a context manager holding the canvas-writer lock that turns an acquisition timeout into 409. Never hand-roll the `get_object_by_uuid` + 404 block or a nested `acquire_canvas_writer` / `try` / `finally` / `release_canvas_writer` sandwich in a route again. Deliberately in `api/`, not `core/`: `core` stays free of `HTTPException` so the job dispatcher and archive importer can use the same primitives. Non-HTTP callers keep using `core/inference_pool/session_lock.py::session_lock`.
- `core/image_files.py` — every read of a session's upload or cumulative background canvas (`get_image_path`, `load_image_bytes`, `load_canvas_bytes`), the BGR/alpha decoders, and the debug click overlay. `core/erase_mask.py` — base64 PNG → boolean mask plus connected-component splitting. Both were carved out of `image_processing.py`, which re-exports every moved name under its historical spelling **because the test suite patches them as `patch("core.image_processing.<name>")`** — that resolves against `image_processing`'s namespace, so it only keeps working while the orchestrators live there and the names stay bound there. Import from the new modules in new code; do not move an orchestrator out without updating those patch targets.
...
- **`core/object_metadata.py`** — one `update_object(uuid, **fields)` is the single write path for an existing object's columns; the eight `set_object_*` wrappers it replaced were each a dict literal plus a log line, and `set_object_average_depth` had no callers at all. `reset_object_transform` survives because it names a policy (the creation-default column set), not a field, and `ROTATION_POSE_CLEARED` is the shared constant both it and `DELETE /objects/{uuid}/rotation` write. One consequence worth knowing: `PATCH /images/objects/{uuid}` is now atomic — it used to issue one DB round-trip per field group (name, offset, scale, css) against the same row. Otherwise the same public functions as before (`save_object_metadata`, `get_object_by_uuid`, `list_object_ids`, `build_clone_metadata`, …) but each opens its own short DB session instead of reading/writing a JSON file; `base_dir`/`storage_dir` parameters were dropped since metadata no longer lives under the image storage dir. `list_object_ids` / `next_object_id` moved here from `core/object_storage.py` (they used to scan the filesystem; now they query the `objects` table — the real fix for a `next_object_id` dir-scan race described below).
...
- **The depth-adapted pass is mandatory; it is not the only pass.** RGB alone over-segments on fabric creases and shadows — that is why `SamImageAdapter` exists and why depth candidates are returned first. The legacy `ObjectRemover` path (step 4 above) feeds SAM the adapted depth map *only*. The live UI path, `ObjectSegmentor.get_mask_for_object_at_position`, runs SAM **twice**: Pass A on the adapted depth map, Pass B on the raw RGB, returning both candidate sets concatenated (depth first). `multimask_output=True` yields 3 candidates per pass (roughly small/tight/broad), so the mask picker sees **up to 6**. Never drop Pass A in favor of RGB alone.
...
- **Stylesheets are one file per screen or widget** under `src/styles/`, with `src/style.css` as the `@import` index. **The import order in that index is the cascade order and is load-bearing** — `base.css` defines the tokens everything else reads, and `responsive.css` overrides the layout rules above it. Vite inlines the imports, so this still ships as a single stylesheet. Add a new section as a new file plus one `@import` line in the right position, never by appending to another section's file.
- **Design tokens** live at the top of `src/styles/base.css`: graphite chrome (`--chrome-*`), cyan accent (`--cyan`, `--cyan-bright`), IBM Plex Sans for UI and IBM Plex Mono for counters/status readouts (loaded in `index.html`). Radii stay at 2–3 px throughout.
