...
_logger = logging.getLogger("avroom_object_removal.openlrm_v10")
...
class LRMInferrer:
    ...
    def _load_checkpoint(self, model_name: str, cache_dir: str | None = None):
        # download checkpoint if not exists
        if cache_dir is None:
            cache_dir = os.environ.get(
                "OPENLRM_WEIGHT_CACHE",
                str(Path.home() / ".cache" / "avroom_openlrm"),
            )
        ...
        # weights_only=False: checkpoint bundles arbitrary Python objects (legacy).
    ...
    def _build_model(self, model_kwargs, model_weights):
        ...
        _logger.info("Loaded OpenLRM model from checkpoint")
        return model
    ...
    def infer(self, source_image: str, dump_path: str, source_size: int, render_size: int, mesh_size: int, export_video: bool, export_mesh: bool):
        ...
        if 'frames' in results:
            ...
            for k, v in renderings.items():
                if k == 'images_rgb':
                    ...
                    pass
            _logger.debug("Video dumped to %s", dump_path)
        ...
        if 'mesh' in results:
            ...
            _logger.debug("Mesh dumped to %s", dump_path)
