...
# ponytail: planar CSS yaw assumes this HFOV so 3D wall-to-wall ~90°
# becomes the photo's apparent rotateY (~64°). Replace with GeoCalib fx.
_PLANAR_CSS_HFOV_DEG = 70.0
...
def _pixel_yaw_deg(
    x: int,
    image_width: int,
    hfov_deg: float = _PLANAR_CSS_HFOV_DEG,
) -> float:
    """Horizontal camera-ray yaw at pixel ``x`` for a centered pinhole."""
    if image_width <= 1:
        return 0.0
    cx = (image_width - 1) / 2.0
    focal = (image_width / 2.0) / math.tan(math.radians(hfov_deg) / 2.0)
    return math.degrees(math.atan2(float(x) - cx, focal))
...
def orbit_pose_from_normals(
    ...
    *,
    ...
    Deadzone (same wall / tiny delta) returns identity ``(0, 0)`` so a later
    drop back onto the origin wall can reset CSS / mesh pose. ``None`` is only
    for floor-standing sources, which must not be auto-tilted.

    ``wall_mount`` applies the ±90° mesh pitch used by novel-view. Planar CSS
    rotateX/Y must skip that — a painting needs yaw between walls, not a
    top-down camera. Planar also adds dest-pixel camera-ray yaw so a 90°
    world corner maps to the photo's apparent CSS rotateY.
    """
    ...
    if math.hypot(delta_az, delta_el) < _ORBIT_DEADZONE_DEG:
        ...
        return OrbitPose(azimuth_deg=0.0, relative_elevation_deg=0.0)

    if not wall_mount and dest_x is not None and image_width is not None:
        delta_az = _wrap_azimuth_delta(delta_az + _pixel_yaw_deg(dest_x, image_width))
