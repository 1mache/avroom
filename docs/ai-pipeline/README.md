...
The pipeline is the Python package `avroom_object_removal` ([`ai-pipeline/src/`](../../ai-pipeline/src/)). Its main job is **click-based object removal** in one pass over the image.
...
- **In the package but not on the click API path:** 3D reconstruction (Hunyuan3D-2.1 by default with automatic TripoSR fallback; OpenLRM/Trellis/VFusion3D when injected explicitly) — wired into `POST /3d/test-3d` and `POST /images/novel-view`, not `/images/click`.
