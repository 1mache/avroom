...
6. **Compose onto original** — `BackgroundInpainter` copies `original_image` and replaces only compose-mask pixels (typically cutout alpha / raw SAM mask) with the model output; non-compose pixels stay unchanged. Inpainting still uses the broader `refined_mask`. Padding is controlled by `COMPOSE_MASK_PADDING_RADIUS`.
