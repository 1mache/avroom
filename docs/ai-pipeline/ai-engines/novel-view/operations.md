...
`POST /images/novel-view` renders the exact resolved pose fresh from the object's GLB on every call — `MeshRenderNovelViewStrategy` is a direct mesh render, cheap enough that there is nothing worth caching per angle, and no pose quantization/snapping happens either. The response echoes the exact resolved values, not a quantized approximation.
