...
- **`Reconstruction3DFacade`** — Wraps exactly one `Reconstruction3DStrategy`; forwards `generate(...)`. Defaults to `OpenLrmReconstructionStrategy`; pass another strategy at construction to swap.
...
- **`OpenLrmReconstructionStrategy`** — Default backend: local PyTorch + vendored OpenLRM inference; lazy-loaded inferrer. Raises **`OpenLrmReconstructionError`** on inference/export failures.
