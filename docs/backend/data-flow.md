...
sequenceDiagram
    ...
    Core->>AI: cut_mask_from_image(original, refined_mask)
    AI-->>Core: background_bgr
    Router->>Disk: write {uid}_background.png + {uid}_cutout.png
...
- Successful inpaint promotes selected cutout to final `{uid}_cutout.png`.
