...
| **Strategy** | `DepthMappingStrategy`, `ImageSegmentationStrategy`, `ImageInpaintingStrategy`, `NovelViewStrategy`, `Reconstruction3DStrategy`, `SegmentationRoutingStrategy` (ABCs) | Make the "which model / which algorithm" decision pluggable. The concrete strategies live under each domain's `strategies/` folder. |
