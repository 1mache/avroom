...
- `debugDepthMap(file, options)` and `debugSamEverything(file, options)` both post multipart `file` with knobs in the query string (`URLSearchParams`) to `POST /debug/depth-map` / `POST /debug/sam-everything`, and read the response as a `Blob` via the shared `postForDebugImage` helper. Both return a `DebugImageResult { objectUrl, elapsedMs, maskCount }` — `objectUrl` is `URL.createObjectURL(blob)`, `elapsedMs`/`maskCount` come from the `X-Elapsed-Ms`/`X-Mask-Count` response headers (`null` if a header is absent, which only happens for `maskCount` on the depth-map endpoint).
