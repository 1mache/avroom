...
- `handleCut` arms `cutMode` (and clears rotate mode / any pick point); the next stage click fires the segment request (`verifyMode` from the toolbar radio) and disarms itself. Escape cancels while `cutMode || rotateMode` is true (global keydown effect, ignored while focus is in an `<input>`/`<textarea>`). `verifyMode` defaults to `"manual"` (picker); `"auto"` skips the picker and inpaints the CLIP-selected mask.
