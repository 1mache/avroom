...
The toolbar area tool arms a box drag on the stage (always `verify=auto`). Mouse-up calls `POST /images/{uid}/batch` with `source.kind=box`. Ctrl/Cmd-click rail thumbs plus the rail **3D** button send `source.kind=objects`. One batch at a time on the client. Results land through `useSessionSync` after `onMutated`.
...
- **Backtrack / Forward**: toolbar buttons (and Ctrl/Cmd+Z / Ctrl+Shift+Z / Ctrl/Cmd+Y) call `POST /images/{uid}/history/undo` and `.../redo`. Undo restores the previous background stage and hides objects created after that stage; redo brings them back. Making a new inpaint/erase after undoing dumps the forward branch. Up to four prior stages are kept server-side.
