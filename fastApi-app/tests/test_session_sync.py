...
from core.repositories.session_repo import SessionNotFoundError, evaluate_session_sync
...
def test_touch_session_writes_and_updates_timestamp(storage_sandbox: Path) -> None:
    ...
    assert second
    assert second >= first
    assert session_repo.get_session_last_changed(uid) == second
...
def test_clear_session_last_changed_removes_entry(storage_sandbox: Path) -> None:
    ...
    assert session_repo.get_session_last_changed(uid) is None


def test_sync_check_matching_timestamp(storage_sandbox: Path) -> None:
    uid = "session-c"
    _register_session(uid)
    last_changed = session_repo.touch_session(uid)
...
def test_set_name_advances_last_changed(storage_sandbox: Path) -> None:
    uid = "session-e"
    _register_session(uid)
    initial = session_repo.touch_session(uid)
    ...
    session_repo.set_session_name(uid, "Living room")
    updated = session_repo.touch_session(uid)

    assert updated != initial
    assert session_repo.get_session_last_changed(uid) == updated
